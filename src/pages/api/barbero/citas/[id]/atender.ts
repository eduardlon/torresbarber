...
import { iniciarAtencionCita } from '../../../../../services/citaManagementService';
...
export const POST: APIRoute = async ({ params, cookies }) => {
  try {
    ...
    const cita = await iniciarAtencionCita({ citaId, barberoId: session.barbero.id });

    return jsonResponse({ success: true, cita });
  } catch (error) {
    const status = error instanceof Error && 'status' in error ? (error as any).status ?? 500 : 500;
    console.error('[api/barbero/citas/:id/atender] Error:', error);
